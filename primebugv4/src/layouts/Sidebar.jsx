...
import { FaChartPie, FaUsers, FaFolderOpen, FaBug, FaChevronDown, FaArrowLeft, FaFileAlt, FaSignOutAlt, FaChartLine, FaTimes } from 'react-icons/fa';
...
const Sidebar = ({ isOpen, setIsOpen }) => {
    ...
    return (
        <>
            {isOpen && <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[50] lg:hidden animate-fade-in" onClick={() => setIsOpen(false)} />}

            <aside className={`fixed left-0 top-0 h-screen w-64 bg-slate-900 text-slate-300 flex flex-col border-r border-slate-800 shadow-xl z-[55] transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
                <div className="p-5 border-b border-slate-800 bg-slate-900">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white text-lg shadow-lg"><FaBug /></div>
                            <h1 className="text-white font-bold text-lg tracking-tight">PrimeBug</h1>
                        </div>
                        <button onClick={() => setIsOpen(false)} className="lg:hidden text-slate-500 hover:text-white p-1"><FaTimes size={18} /></button>
                    </div>

                    <div className="relative">
                        <select value={currentTeam?.id || ''} onChange={(e) => switchTeam(e.target.value)} className="w-full bg-slate-800 text-white text-sm font-medium py-2 px-3 rounded-lg border border-slate-700 appearance-none cursor-pointer">
                            {userTeams.length > 0 ? userTeams.map(t => <option key={t.id} value={t.id}>{t.nombre}</option>) : <option>Sin Equipo</option>}
                        </select>
                            ...
                            <div className="p-3 space-y-1">
                                {projectMenu.map((item) => (
                                    <Link 
                                        ...
                                        className={`flex items-center gap-3 px-3 py-3 rounded-lg text-sm ${isActive(item.path, item.exact) ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-800 hover:text-white'} ${item.path.includes('/bugs') ? 'tour-tab-bugs' : ''}`}
                                    >
                        ...
                        <div className="space-y-1 p-3">
                            ...
                            {globalMenu.map((item) => (
                                <Link 
                                    ...
                                    to={item.path} 
                                    onClick={() => setIsOpen(false)} 
                                    className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm ${isActive(item.path) ? 'bg-indigo-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white'} ${item.path === '/proyectos' ? 'tour-select-project' : ''}`}
                                >
                                    {item.icon} <span>{item.label}</span>
                                </Link>
                            ))}
