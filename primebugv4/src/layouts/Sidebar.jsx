...
/**
 * COMPONENTE AUXILIAR: SidebarSeparator
 */
        ...
        <aside className={`fixed top-0 left-0 h-full bg-slate-900 text-white transition-all duration-300 z-30 shadow-2xl ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
            <div className="flex flex-col h-full">
                ...
                <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto custom-scrollbar">
                    ...
                    <NavItem to="/equipos" icon={<FaUsers />} isSidebarOpen={isSidebarOpen}>Equipos</NavItem>
                    ...
                    {/* CONTEXTUAL PROJECT MENU - CORREGIDO Y AMPLIADO */}
                    {projectId && (
                        <div className="animate-fade-in">
                            ...
                            {isSidebarOpen && (
                                <div className="px-6 mb-2">
                                    ...
                                    <p className="text-sm font-bold text-indigo-400 truncate mt-1">{projectName || 'Cargando...'}</p>
                                </div>
                            )}
                ...
                <div className="px-3 py-4 border-t border-slate-700/50 bg-slate-900/50">
                     <div className={`flex items-center ${isSidebarOpen ? 'gap-x-3 px-3' : 'justify-center'}`}>
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-black text-white shadow-lg border border-white/10 flex-shrink-0">
                            {profile?.nombre_completo ? profile.nombre_completo.charAt(0) : (user?.email?.charAt(0).toUpperCase() || 'U')}
                        </div>
                        {isSidebarOpen && (
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-white truncate leading-tight">{profile?.nombre_completo || 'Usuario'}</p>
